...
<div class="container">
    <table id="dg" style="width:100%;height:529px" title="盘点记录查询列表" data-options="
        ...
        <thead>
        <tr>
            <th field="code" width="15%">盘点编号</th>
            <th field="name" width="20%">盘点名称</th>
            <th field="date" width="20%">盘点日期</th>
            <th field="shopname" width="15%">分店名字</th>
            <th field="status" width="15%">盘点状态</th>
            <th field="fexp" width="15%">盘点备注</th>
        </tr>
        </thead>
    </table>
...
<script type="text/javascript">
    ...
    function tableData(form) {
        ...
        $.ajax({
            type: "POST", url: "/Checkmain/selectByAll", dataType: "json", data: data, success: function (json) {
                for (var i = 0; i < json.list.length; i++) {
                    var checkmain = json.list[i];
                    rows.push({
                        id: checkmain.id,
                        st: checkmain.status,
                        code: checkmain.checkcode,
                        name: checkmain.checkname,
                        date: checkmain.checkdate,
                        shopname: checkmain.shopid.name,
                        status: checkmain.statusString,
                        fexp: checkmain.fexp
                    });
                }
                //页数相关赋值
    ...
    function resetDg() {
        ...
        $("#dg").datagrid({
            onDblClickRow:function(rowIndex, rowData){
                //编辑本行资料
                showPage("修改库存盘点","/Checkmain/updat?id="+rowData.id,1100,450,function(){
                    tableData();
                },false,false);
            }
        });
    ...
    //打开新增页面
    function addPage() {
        showPage("新增库存盘点","/Client/ad",1000,400,function(){
            tableData();
        },true,false);
    }
    ...
    function getSelects() {
        ...
        $.getJSON("/Checkmain/getStatus", function (json) {
            //获得client查询变量
            var tremOpt = json.checkmainInput;
    ...
    function changeInput(select) {
        ...
        $.getJSON("/Checkmain/getStatus", function (json) {
            //获得client查询变量
            var textOpt = json.checkmainInput[selval].input;//下拉框集合
            //如果是空的，则是输入框
