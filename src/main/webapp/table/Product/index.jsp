...
<div class="container">
    <table id="dg" style="width:100%;height:529px" title="商品列表" data-options="
        ...
        <thead>
        <tr>
            <th field="code" width="14%">编号</th>
            <th field="name" width="14%">名称</th>
            <th field="mode" width="14%">型号</th>
            <th field="status" width="14%">状态</th>
            <th field="producttypeid" width="14%">商品类型名称</th>
            <th field="userid" width="15%">操作员名称</th>
            <th field="fexp" width="15%">商品备注</th>
        </tr>
        </thead>
    </table>
...
<script type="text/javascript">
    ...
    function tableData(form) {
        ...
        $.ajax({
            type: "POST", url: "/Product/selectByAll", dataType: "json", data: data, success: function (json) {
                for (var i = 0; i < json.list.length; i++) {
                    var product = json.list[i];
                    rows.push({
                        id: product.id,
                        code: product.code,
                        name: product.name,
                        producttypeid: product.producttypeid.name,
                        status: product.statusString,
                        userid: product.userid.name,
                        fexp: product.fexp
                    });
                }
                //页数相关赋值
    ...
    function resetDg() {
        ...
        $("#dg").datagrid({
            onDblClickRow:function(rowIndex, rowData){
                ...
                //编辑本行资料
                showPage("修改产品信息","/Product/updat?id="+rowData.id,1000,400,function(){
                    tableData();
                },true,false);
            }
        });
    ...
    //打开新增页面
    function addPage() {
        showPage("新增产品信息","/Product/ad",1000,400,function(){
            tableData();
        },true,false);
    }
