...
<div id="test"></div>
<div class="container">
    <table id="dg" style="width:100%;height:529px" title="产品类型列表" data-options="
            rownumbers:true,
            singleSelect:false,
            autoRowHeight:true,
            pagination:false,
...
<script type="text/javascript">
    ...
    function tableData(form) {
        ...
        $.ajax({
            type: "POST", url: "/Producttype/selectByAll", dataType: "json", data: data, success: function (json) {
                for (var i = 0; i < json.list.length; i++) {
                    var producttype = json.list[i];
                    rows.push({
                        id: producttype.id,
                        name: producttype.name,
                        fexp: producttype.fexp
                    });
                }
                //页数相关赋值
                var jsea=json.sea;
    ...
    function resetDg() {
        ...
        $("#dg").datagrid({
            onDblClickRow:function(rowIndex, rowData){
                ...
                //编辑本行资料
                showPage("修改产品类型信息","/Producttype/updat?id="+rowData.id,1000,400,function(){
                    tableData();
                },true,false);
            }
        });
    ...
    //打开新增页面
    function addPage() {
        showPage("新增产品类型信息","/Producttype/ad",1000,400,function(){
            tableData();
        },true,false);
    }
    ...
    function getSelects() {
        ...
        $.getJSON("/Producttype/getStatus", function (json) {
            //获得producttype查询变量
            var tremOpt = json.producttypeInput;
    ...
    function changeInput(select) {
        ...
        $.getJSON("/Producttype/getStatus", function (json) {
            //获得producttype查询变量
            var textOpt = json.producttypeInput[selval].input;//下拉框集合
            //如果是空的，则是输入框
